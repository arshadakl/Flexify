...
import { submitOrderWork } from "../../common/utils/APIs/FreelancerApi";
import { useNavigate } from "react-router-dom";
import { FormatDateString } from "../../common/utils/Services/dateFormater";
import { toast } from "sonner";

function SubmitionComponet({work}: {work:any}) {
  ...
  return (
    <>
      { work && <div className="bg-white border px-5  py-5 ">
        <h1 className="text-md text-2xl mx-5">
          ID : <span className="font-medium text-gray-500">#W123151455212</span>
        </h1>
        <p className="mx-5 ">
          <i className="fa-duotone fa-briefcase " /> Client : {work.client[0].username}
        </p>
        <hr className="my-3" />
            ...
            <tbody>
              <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                ...
                <td className="px-6 py-4 ">
                  {FormatDateString(work.deadline)}
                </td>
                <td
                  className={`px-6 py-4 font-semibold capitalize ${
                    work.status !== "submited"
                      ? "text-yellow-600"
                      : "text-green-600"
                  }`}
                >
...
{(work.status!=="submited" && work.status!=="Awaiting Approval") ? <>
            ...
            <p className="text-[12px]">
              {file !== "" ? (
                <span className="text-green-600">
                  <i className="fa-light fa-circle-check mx-1"></i>File Selected
                </span>
              ) : (
...
export default SubmitionComponet;
