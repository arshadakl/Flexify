...
function SinglePage() {
  ...
  // }
  const resons = ["Misrepresenting Skills or Experience","Unrealistic Scope or Budget","Inappropriate Content","Copyright Infringement","Suspicious Activity"]
  return (
    <>
  {isLoad ? <Loading/> : null}
    ...
    data-modal-toggle="select-modal"
              ...
              >
                <p className="w-3/5 text-end"><i className="fa-light fa-bug mx-1"/> Report Work</p>
              </button>
            </div>
          </div>
        </div>
    ...
    <div className="relative p-4 w-full max-w-md max-h-full">
      ...
      <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
        ...
        <div className="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
          ...
          <i className="fa-regular fa-message-exclamation mx-1" /> Reasons for Reporting
          </h3>
          <button  
            type="button" ref={closeBTN}
...
<li key={index}>
          ...
          <button disabled={selectedReson=="" ? true : false} onClick={handilRepot} className={`text-white inline-flex w-full justify-center ${ selectedReson=="" ? 'bg-slate-200' : 'bg-gray-900 hover:bg-gray-700'} focus:ring-4 focus:outline-none  font-medium rounded-lg text-sm px-5 py-2.5 text-center  `}>
