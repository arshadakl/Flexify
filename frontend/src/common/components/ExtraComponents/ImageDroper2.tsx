...
function ImageDroper2({ previewImage, setPreviewImage, croppedImage, setCroppedImage }:{previewImage:any, setPreviewImage:any, croppedImage:any, setCroppedImage:any}) {
  // const [previewImage, setPreviewImage] = useState<string | null>(null);
  // const [croppedImage, setCroppedImage] = useState<string | null>(null);
  const [showModal, setShowModal] = useState<boolean>(false);
  const imageRef2 = useRef<HTMLImageElement>(null);
  const cropperRef2 = useRef<Cropper>();

  useEffect(() => {
    if (imageRef2.current && previewImage && showModal) {
      cropperRef2.current = new Cropper(imageRef2.current, {
    ...
    return () => {
      if (cropperRef2.current) {
        cropperRef2.current.destroy();
        cropperRef2.current = undefined;
      }
    };
  ...
  return (
    <div>
      <div className="flex items-center justify-center w-full">
        ...
        >
          {(previewImage) ? (<>
            ...
            <img ref={imageRef2} src={croppedImage ? croppedImage : previewImage} alt="Preview" className="w-full h-full object-cover" />
        ...
        <div className="fixed inset-0  flex items-center justify-center bg-gray-900 bg-opacity-75 z-50">
          <div className="bg-white p-4 rounded-lg w-3/6">
            <div className="relative">
              <img ref={imageRef2} src={previewImage || ''} alt="Cropping Preview" style={{maxHeight:"80vh"}} className="w-full h-full object-cover" />
              <button
                className="absolute top-2 right-2 bg-red-700 text-white px-2 py-1 rounded-md hover:bg-red-800"
      ...
      {/* {croppedImage && (
        <div className="flex justify-center mt-4">
          <img src={croppedImage} alt="Cropped Image" className="max-w-full max-h-screen" />
        </div>
      )} */}
    </div>
  );
