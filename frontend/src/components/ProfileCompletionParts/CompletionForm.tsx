...
import { profileCompletion } from "../../API/FreelancerApi";
...
function Test() {
  ...
  const { userId, setFreelancerDetails } = useContext(AuthContext);
  ...
  const updateSkills = () => {
    const skillsArray = skills.map((item) => item.skill);
    setFormData({
      ...formData,
  ...
  const addSkills = () => {
    ...
    setSkills((prevSkills) => [
      ...prevSkills,
      { id: Date.now(), skill: input },
    ]);
    console.log(skills);
    setInput("");
    updateSkills(); // Call updateSkills when adding a skill
  };

  const removeSkill = (id: number) => {
    console.log("remove called");
    const updatedSkill = skills.filter((item) => item.id !== id);
    console.log(updatedSkill);
    setSkills(updatedSkill);
    updateSkills(); // Call updateSkills when removing a skill
  };
              ...
              <div className="w-screen bg-white  md:mt-0 sm:max-w-lg  xl:p-0 dark:bg-gray-800 dark:border-gray-700">
                <div className="p-6 space-y-1 md:space-y-6 sm:p-8">
                  ...
                  <div className=" w-80 m-1">
                    ...
                    <input type="text" value={formData.firstName} name="firstName" onChange={handleChange} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="first name" required />
                  </div>
                  <div className="w-80 m-1">
                    <label className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                      Last Name
                    </label>
                    <input value={formData.lastName} name="lastName" onChange={handleChange} type="text" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="last name" required />
                  </div>
                </div>
                ...
                <div className="w-full flex justify-between">
                  <div className=" w-80 m-1">
                    ...
                    <select
                      id="countries" value={formData.Country} name="Country" onChange={handleChange}
                  ...
                  <div className="w-80 m-1">
                    ...
                    <select
                      id="countries" value={formData.language} name="language" onChange={handleChange}
                      className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    >
                      <option selected disabled>
